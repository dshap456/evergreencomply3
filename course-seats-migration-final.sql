-- Course Seat Management Migration (Final Version)
-- This version handles all edge cases including view column type conflicts

-- 1. Create course_seats table if it doesn't exist
...
CREATE INDEX IF NOT EXISTS idx_course_seats_account_id ON public.course_seats(account_id);
CREATE INDEX IF NOT EXISTS idx_course_seats_course_id ON public.course_seats(course_id);

-- Enable RLS
ALTER TABLE public.course_seats ENABLE ROW LEVEL SECURITY;

-- 2. Create course_invitations table if it doesn't exist
...
-- Add indexes if they don't exist
CREATE INDEX IF NOT EXISTS idx_course_invitations_account_id ON public.course_invitations(account_id);
CREATE INDEX IF NOT EXISTS idx_course_invitations_course_id ON public.course_invitations(course_id);
CREATE INDEX IF NOT EXISTS idx_course_invitations_invite_token ON public.course_invitations(invite_token);

-- Enable RLS
ALTER TABLE public.course_invitations ENABLE ROW LEVEL SECURITY;

-- 3. Add tracking columns to course_enrollments if they don't exist
...
DROP POLICY IF EXISTS "course_seats_select" ON public.course_seats;
DROP POLICY IF EXISTS "course_seats_insert" ON public.course_seats;
DROP POLICY IF EXISTS "course_seats_update" ON public.course_seats;
DROP POLICY IF EXISTS "course_seats_delete" ON public.course_seats;
...
-- 5. Drop existing policies if they exist and recreate them for course_invitations
...
-- 6. Create or replace function to get available seats for a course
...
-- 7. Create or replace function to accept course invitation
...
-- 8. Drop the existing view if it exists and recreate it with proper types
...
SELECT 
    ...
    ce.final_score,
    u.email::varchar as user_email,  -- Cast to varchar without length constraint
    ...
    inv.email::varchar as inviter_email  -- Cast to varchar without length constraint
...
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'course_seats') THEN
        RAISE NOTICE 'Table course_seats exists ✓';
    ELSE
        RAISE WARNING 'Table course_seats NOT FOUND ✗';
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'course_invitations') THEN
        RAISE NOTICE 'Table course_invitations exists ✓';
    ELSE
        RAISE WARNING 'Table course_invitations NOT FOUND ✗';
    END IF;
    
    IF EXISTS (SELECT FROM information_schema.views WHERE table_name = 'team_course_enrollments') THEN
        RAISE NOTICE 'View team_course_enrollments exists ✓';
    ELSE
        RAISE WARNING 'View team_course_enrollments NOT FOUND ✗';
    END IF;
END $$;
