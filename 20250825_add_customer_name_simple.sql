...
-- Date: 2025-08-25
-- Version: Simplified to work with actual table structure
...
-- Step 4: Update the process_course_purchase_by_slug function
-- First, let's check the current function signature
-- Then update it to accept the new parameter
CREATE OR REPLACE FUNCTION process_course_purchase_by_slug(
    p_course_slug text,
    p_account_id uuid,
    p_payment_id text,
    p_quantity integer DEFAULT 1,
    p_customer_name text DEFAULT NULL  -- NEW PARAMETER
) RETURNS json AS $$
DECLARE
...
BEGIN
    ...
    IF enrollment_record IS NULL AND p_quantity = 1 THEN
        ...
        INSERT INTO course_enrollments (
            user_id,
            course_id,
            account_id,  -- This exists from a previous migration
            enrolled_at,
        ...
        VALUES (
            p_account_id,
            course_record.id,
            p_account_id,  -- Use the same ID for account_id
            NOW(),
...
-- Step 5: Create trigger to update name when invitations are accepted
...
BEGIN
    ...
    SELECT COUNT(*) INTO named_enrollments FROM course_enrollments 
    WHERE customer_name IS NOT NULL AND customer_name != 'Name Not Recorded';
    SELECT COUNT(*) INTO unnamed_enrollments FROM course_enrollments 
    WHERE customer_name = 'Name Not Recorded' OR customer_name IS NULL;
