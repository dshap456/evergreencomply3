...
const MIGRATION_SQL = `
-- Create a new function that processes course purchases by slug
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ 
        message: 'Migration might need to be run manually',
        error: error.message,
        suggestion: 'Run this SQL in Supabase dashboard',
        sql: MIGRATION_SQL
      });
    }
    ...
    return NextResponse.json({ 
      ...
      function_created: 'process_course_purchase_by_slug'
    });
  } catch (error) {
    return NextResponse.json({ 
