-- Fix the type mismatch between UUID and VARCHAR in token comparisons
-- The pending_invitation_tokens.invitation_token is UUID but course_invitations.invite_token is VARCHAR

-- First, alter the pending_invitation_tokens table to use VARCHAR instead of UUID
...
-- Now fix the process_pending_course_invitation function to handle the type properly
...
BEGIN
  ...
  -- Find pending invitation token for this email (now both are VARCHAR)
  SELECT invitation_token::VARCHAR
  ...
  -- Process the course invitation using the token we found (both are VARCHAR now)
  SELECT ci.*
  INTO v_invitation
...
-- Also create the token-based version for better reliability
...
BEGIN
  ...
  -- Find the course invitation by token directly (both are VARCHAR)
  SELECT ci.*
  INTO v_invitation
  FROM public.course_invitations ci
